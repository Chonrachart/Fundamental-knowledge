...
1. Check static pod manifest: `cat /etc/kubernetes/manifests/etcd.yaml` — verify cert paths and `--data-dir`.
2. Check kubelet logs: `journalctl -u kubelet -n 50` — kubelet manages static pods.
3. Verify cert files exist: `ls /etc/kubernetes/pki/etcd/`.

### API server cannot connect to etcd

1. Check etcd is healthy: `ETCDCTL_API=3 etcdctl endpoint health ...`.
2. Check API server config: `cat /etc/kubernetes/manifests/kube-apiserver.yaml` — `--etcd-servers` flag.
3. Check network: etcd listens on port 2379 (client) and 2380 (peer); verify not blocked by firewall.

### etcd cluster lost quorum

1. If majority of nodes are down, etcd goes read-only — `kubectl` reads work, writes fail.
2. Restore from backup on a single node, then re-add others.
3. See `05-cluster-maintenance/003-etcd-backup-restore.md`.
