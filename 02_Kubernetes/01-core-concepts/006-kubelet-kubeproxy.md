...
- **What it is** — kubelet watches the API server,then load and unload container as instructed by scheduler and send back report status of pod on them
- **One-liner** — contact point with kubeapi on workernode and monitored pod in node 
...
3. Kubelet then instructs the container runtime      
