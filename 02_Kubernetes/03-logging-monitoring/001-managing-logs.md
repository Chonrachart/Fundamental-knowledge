...
2. Pod was deleted: logs are lost unless captured by a logging system.
3. Container has not written to stdout/stderr; app may be logging to file instead.
4. Verify pod is actually running: `kubectl exec <pod> -- ps aux` to check for active processes.

### Container restarted; need logs from before the crash
1. Use `--previous` to retrieve logs from the last container instance: `kubectl logs <pod> --previous`.
2. If pod restarted multiple times, you can only see the last instance; cluster-level logging is needed for history.
3. Check exit code: `kubectl describe pod <pod>` — Last State shows why container exited.

### Container did not restart but is not writing logs
1. Check if container is actually running: `kubectl get pod <pod> -o wide`.
2. App may be logging to a file instead of stdout; use `kubectl exec <pod> -- tail /app/logs/*.log`.
3. Check app configuration; verify log level is not set to SILENT or ERROR only.

### Multi-container pod; cannot tell which container is logging
1. Check all containers: `kubectl logs <pod> --all-containers=true`.
2. Get specific container: `kubectl logs <pod> -c <container-name>`.
3. Use `kubectl describe pod <pod>` to see which containers are Running/Waiting/Terminated.

### Need logs from all pods in a Deployment
1. Use label selector to get all pods: `kubectl logs -l app=myapp --all-containers=true`.
2. Logs will be interleaved; for structured debugging, use a logging backend with timestamp sorting.
3. If pods are spread across namespaces, add `-A` (all namespaces).

### Logs are being written too fast or too slow
1. **Too fast, disk filling up:** Set up log rotation on nodes; kubelet has `--container-log-max-size` and `--container-log-max-files` flags.
2. **Too slow/missing logs:** If logs arrive after pod deletion, they are lost; deploy a logging backend to capture in real-time.

### Cannot find logs for a pod that was deleted
1. Deleted pods lose their logs unless a logging system captured them.
2. Set up cluster-level logging (DaemonSet or sidecar) to ensure logs persist.
3. Check historical logs: `kubectl get events -A --sort-by=.lastTimestamp` to see pod lifecycle events.
