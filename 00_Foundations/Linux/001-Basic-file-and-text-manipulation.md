...
```bash
cat /var/log/syslog | grep "error" | cut -d' ' -f5 | sort -u
```

- `cat` reads file → stdout → pipe
- `grep` filters lines → stdout → pipe
- `cut` extracts field 5 → stdout → pipe
- `sort -u` deduplicates → terminal
