...
- **Why it exists** — Manual copy --> inefficient and error-prone. chaos from manual backups(have many backups).
- **What it is** — Tracks changes in files and data that is immutable. Maintains a history of who, what, when. every commit is content-addressed (hash). Enables easy rollback to stable version.
...
- Note: If you clone a repository (including inside WSL), you do not need to run git init because the repo is already initialized.
...
### Three Git Areas
- **What it is** — Commit does NOT read from Working Directory. It **reads only from Index**.
...
### .gitignore
- **What it is** — A file that tells Git which files or directories should NOT be tracked. Affects only untrack files. Does NOT remove files from your system, it only prevents Git from tracking them.
