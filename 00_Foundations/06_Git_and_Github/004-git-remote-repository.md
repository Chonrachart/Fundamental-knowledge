...
# Overview
- **What it is** — Each developer has a full copy of the repository locally. Sync work between local & remote repository through a Git server.
...
- Download full repository to your local machine and create folder with repository name.
...
- Remote branches(e.g., origin/main, origin/featureA) represent the state of branches on the remote server.
...
- Equivalent to: `git fetch <remote>` + `git merge <upstream-branch>`

### Git Rebase
- **Why it exists** — Avoids creating merge commits by rewriting history into a linear sequence.
- **What it is** — Take my `current branch` commits and replay them on top of `branch`. change the **base commit** of a branch. **!!! Do NOT rebase public/shared branches that have already been pushed.**
