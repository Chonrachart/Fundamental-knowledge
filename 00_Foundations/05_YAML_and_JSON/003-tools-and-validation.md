...
Problem: config file fails validation or tools produce unexpected output
    |
    v
[1] Is it valid YAML/JSON at all?
    yamllint file.yml  /  python3 -m json.tool < file.json
    |
    +-- syntax error --> fix tabs, indentation, commas, quotes
    |
    v
[2] Is the data structure what you expect?
    yq '.' file.yml  /  jq '.' file.json   # view parsed structure
    |
    +-- wrong nesting --> check indentation (YAML) or brace matching (JSON)
    +-- wrong type --> check boolean trap, unquoted numbers
    |
    v
[3] Does the tool's schema accept your file?
    kubectl apply --dry-run=client -f file.yml
    ansible-playbook --syntax-check playbook.yml
    |
    +-- unknown field --> check API version, typo in field name
    +-- wrong type --> expected string got int, or vice versa
    |
    v
[4] Is yq/jq query returning nothing?
    check path step by step: .a then .a.b then .a.b.c
    use 'keys' (jq) or 'keys' (yq) to list available fields
    |
    v
[5] Encoding issues?
