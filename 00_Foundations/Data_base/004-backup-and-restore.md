...
[1] Did the backup complete?
    Check cron logs / script exit code / file size
    |
    +-- file is 0 bytes or missing --> disk full? permissions? DB down?
    |
    v
[2] Can you connect to the database?
    ...
    psql -c "SELECT 1"
    |
    +-- connection refused --> DB service down, check systemctl status
    +-- access denied --> credentials wrong, check .my.cnf or .pgpass
    |
    v
[3] Restore fails with errors?
    |
    +-- "database does not exist" --> create the target database first
    +-- "role does not exist" --> restore roles first (pg_dumpall -r)
    +-- "table already exists" --> use --clean flag or drop tables first
    +-- version mismatch --> pg_restore/mysql version must match dump version
    |
    v
[4] Data looks wrong after restore?
    |
    +-- row counts differ --> was backup taken during heavy writes without --single-transaction?
    +-- missing recent data --> backup is from before the change; check PITR options (binlog/WAL)
    |
    v
[5] Restore is too slow?
    |
    +-- use physical backup instead of logical for large DBs
