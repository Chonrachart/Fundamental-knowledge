...
[1] Is the function being called?
    Add print() or breakpoint() at the top of the function body.
    |
    +-- not called --> check spelling, missing parentheses, wrong import
    |
    v
[2] Are the arguments correct?
    Print args/kwargs inside the function.
    |
    +-- wrong values --> check positional vs keyword order, mutable defaults
    |
    v
[3] Is the return value used?
    print(f()) to see what comes back.
    |
    +-- returns None --> missing return statement, or return without value
    |
    v
[4] Scope issue?
    Variable unexpectedly None or wrong value.
    |
    +-- shadowed name --> local variable hides global/enclosing
