...
Problem: replica is lagging or replication is broken
    |
    v
[1] Is replication running?
    MySQL:  SHOW REPLICA STATUS\G  --> check Replica_IO_Running, Replica_SQL_Running
    PG:     SELECT * FROM pg_stat_replication;  (on primary)
    |
    +-- IO thread stopped --> network issue / primary down / auth failure
    +-- SQL thread stopped --> replication error (duplicate key, schema mismatch)
    |
    v
[2] How much lag?
    MySQL:  Seconds_Behind_Source in SHOW REPLICA STATUS
    PG:     replay_lag in pg_stat_replication
    |
    +-- lag growing --> replica too slow (disk I/O, CPU), or primary write load too high
    +-- lag stable but large --> replica fell behind, will catch up if write rate drops
    |
    v
[3] Replication error?
    MySQL:  check Last_SQL_Error in SHOW REPLICA STATUS
    PG:     check PostgreSQL log on replica
    |
    +-- duplicate key / constraint error --> data drift, investigate and fix
    +-- missing table --> schema out of sync, apply DDL on replica
    |
    v
[4] Split-brain suspected?
    |
    +-- check if both nodes accept writes (SELECT pg_is_in_recovery() returns false on both)
    +-- immediately fence one node (stop it, disconnect from network)
    +-- compare data and rebuild the stale node as a replica
    |
    v
[5] Failover not working?
    |
    +-- check failover tool logs (Patroni, Orchestrator, repmgr)
