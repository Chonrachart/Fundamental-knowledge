...
docker run -d --name mysql -e MYSQL_ROOT_PASSWORD=secret -v mysqldata:/var/lib/mysql -p 3306:3306 mysql:8.0
...
docker exec -i mysql mysql -u root -psecret mydb < backup.sql

# check container health and logs
docker inspect --format='{{.State.Health.Status}}' pg
...
kubectl exec -it db-0 -- psql -U postgres
...
[1] Is the container running?
    ...
    |
    +-- not running --> check logs: docker logs <name> / kubectl logs <pod>
    +-- CrashLoopBackOff --> wrong env vars, permission issue, or corrupt data
    |
    v
[2] Can you connect to the database?
    docker exec -it <name> psql -U postgres   (or mysql -u root -p)
    |
    +-- connection refused --> database not fully started yet, check logs
    +-- auth failed --> wrong POSTGRES_PASSWORD / MYSQL_ROOT_PASSWORD
    |
    v
[3] Is data persisting across restarts?
    docker compose down && docker compose up -d
    |
    +-- data gone --> no volume mounted, add -v or volumes: section
    +-- data present --> volumes working correctly
    |
    v
[4] Can the app reach the database?
    docker compose exec app ping db
    kubectl exec <app-pod> -- nc -zv <db-svc> 5432
    |
    +-- host not found --> wrong service name in connection string
    +-- connection refused --> database port not exposed or not ready
    |
    v
[5] Is there enough disk space?
    ...
    |
    +-- disk full --> prune unused images/volumes: docker system prune
    +-- PVC full --> expand PVC (if storage class supports it)
    |
    v
[6] Permission issues?
    docker logs <name> | grep "permission denied"
    |
