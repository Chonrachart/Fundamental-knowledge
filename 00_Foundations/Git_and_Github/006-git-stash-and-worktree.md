...
Stash won't apply cleanly?
  |
  +-> Conflicts? --> resolve conflicts manually --> git add --> git stash drop
  |
  +-> Branch diverged too far? --> git stash branch <new-branch>
  |
  +-> Stash is empty / wrong one? --> git stash list --> git stash show -p stash@{N}

Worktree issues?
  |
  +-> "fatal: '<branch>' is already checked out" --> use a different branch or checkout in only one worktree
  |
  +-> Stale worktree after manual delete? --> git worktree prune
  |
