...
[1] Check the status code
    ...
    |
    +-- 2xx --> success, check response body for expected data
    +-- 4xx --> client error, see subsection above
    +-- 5xx --> server error, not your fault (usually)
    |
    v
[2] Check response body for error message
    Most APIs return: {"error": "description", "code": "ERROR_CODE"}
    |
    v
[3] Verify request format
    - Is Content-Type set correctly?
    - Is the JSON body valid? (echo '...' | jq .)
    - Are required fields present?
    |
    v
[4] Check method correctness
    - Using POST when PUT is needed (or vice versa)?
