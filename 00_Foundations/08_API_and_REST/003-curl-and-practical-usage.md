...
```bash
# GET with auth and parse JSON
curl -s -H "Authorization: Bearer $TOKEN" \
  https://api.example.com/v1/resources | jq '.items[].name'
...
[1] Add -v flag for verbose output
    ...
    |
    +-- connection failed --> DNS? firewall? wrong port?
    +-- TLS error --> certificate issue, use openssl s_client to debug
    |
    v
[2] Check request being sent
    - Correct method? (-X POST vs default GET)
    - Headers present? (-H flags)
    - Body correct? (-d flag, valid JSON?)
    |
    v
[3] Check response status code
    ...
    +-- 3xx --> add -L to follow redirects
    +-- 4xx --> auth or request format issue
    +-- 5xx --> server problem
    |
    v
[4] Parse response body for error details
    curl -s ... | jq .
    |
    v
[5] Compare with known working request
