...
### CI passes but code is broken in production

1. Test coverage gap — add integration tests that cover the failure scenario.
2. Environment mismatch — staging config differs from production.
3. Data-dependent bug — tests use synthetic data, prod has edge cases.
4. Race condition — tests run sequentially, prod runs concurrently.
5. Review deployment strategy — consider canary or blue-green to catch issues early.
