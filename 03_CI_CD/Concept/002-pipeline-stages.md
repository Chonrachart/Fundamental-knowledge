...
5. Check artifact retention — expired artifacts are automatically deleted.

### Stage passes locally but fails in CI

1. Environment difference: check runner OS, tool versions, env vars.
2. Missing dependencies: local machine has tools not installed on runner.
3. File path differences: case sensitivity (Linux CI vs macOS local).
4. Timing: local has warm caches, CI starts cold.
5. Use `act` (local GitHub Actions runner) to reproduce CI environment locally.

### Gate blocks merge unexpectedly

1. Check required status checks in branch protection settings.
2. Verify the check name matches exactly (job name, not workflow name).
