...
1. Variable has no `default` and wasn't set.
2. Set via: `-var`, `-var-file`, `terraform.tfvars`, `*.auto.tfvars`, or `TF_VAR_<name>`.
3. Check variable name spelling — case-sensitive.

### Output not available in parent module
1. Output must be defined in the child module's `.tf` files.
2. Parent references it as `module.<name>.<output_name>`.
3. Run `terraform apply` first — outputs are computed after apply.
