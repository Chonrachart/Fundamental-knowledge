...
### "No configuration files" on init
1. Check you're in the correct directory containing `.tf` files.
2. File extension must be `.tf` (not `.hcl` or `.yaml`).

### Apply succeeds but resource doesn't appear in console
1. Check region: Terraform may have created in a different region than you're viewing.
2. Check tags/name: resource may exist but without expected naming.
3. Verify: `terraform state show <address>` to see actual resource attributes.
