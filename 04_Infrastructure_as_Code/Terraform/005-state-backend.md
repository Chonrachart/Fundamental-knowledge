...
1. Another `terraform apply` may be running — wait for it.
2. If process crashed: `terraform force-unlock <LOCK_ID>`.
3. Only force-unlock if certain no other process holds the lock.

### State file corruption
1. Pull state: `terraform state pull > backup.tfstate`.
2. Check recent changes: was `force-unlock` or manual edit involved?
3. Restore from backend versioning (S3 versioning, GCS versioning).

### "Resource already exists" on apply
1. Resource exists in cloud but not in state — import it: `terraform import <addr> <id>`.
2. If resource shouldn't be managed: remove config block and skip.
3. If duplicate: check for copy-paste errors in resource names.

### Drift — plan shows unexpected changes
